...
const MANIFEST_RELATIVE_PATH: &str = ".openyak-plugin/plugin.json";
...
pub struct PluginToolManifest {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    #[serde(rename = "requiredPermission", alias = "required_permission")]
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("OPENYAK_PLUGIN_ID", &self.plugin_id)
            .env("OPENYAK_PLUGIN_NAME", &self.plugin_name)
            .env("OPENYAK_TOOL_NAME", &self.definition.name)
            .env("OPENYAK_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("OPENYAK_PLUGIN_ROOT", root.display().to_string());
        }
...
impl Display for PluginManifestValidationError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AbsolutePathNotAllowed { kind, path } => write!(
                f,
                "{kind} path `{}` must be relative to the plugin root",
                path.display()
            ),
            Self::PathOutsideRoot { kind, path, root } => write!(
                f,
                "{kind} path `{}` must stay within the plugin root `{}`",
                path.display(),
                root.display()
            ),
...
impl PluginManager {
    ...
    pub fn install(&mut self, source: &str) -> Result<InstallOutcome, PluginError> {
        ...
        let prepared = prepare_plugin_source(&install_source, &temp_root)?;

        let plugin_id = plugin_id(&prepared.manifest.name, EXTERNAL_MARKETPLACE);
        if self
            .list_installed_plugins()?
            .into_iter()
            .any(|plugin| plugin.metadata.id == plugin_id)
        {
            let _ = fs::remove_dir_all(&prepared.prepared_install_path);
            return Err(PluginError::CommandFailed(format!(
                "plugin `{plugin_id}` is already installed; use `/plugins update {plugin_id}` to refresh it"
            )));
        }
        let install_path = self.install_root().join(sanitize_plugin_id(&plugin_id));
        commit_prepared_install(&prepared.prepared_install_path, &install_path, &temp_root)?;
    ...
    pub fn update(&mut self, plugin_id: &str) -> Result<UpdateOutcome, PluginError> {
        ...
        commit_prepared_install(
            &prepared.prepared_install_path,
            &record.install_path,
            &temp_root,
        )?;
        ...
        Ok(UpdateOutcome {
            ...
            new_version: prepared.manifest.version,
            source: prepared.source,
...
fn validate_command_entry(
    ...
    if let Err(error) = validate_manifest_command_path(root, entry, kind) {
        errors.push(error);
    }
}
...
fn validate_command_path(root: &Path, entry: &str, kind: &'static str) -> Result<(), PluginError> {
    if is_literal_command(entry) {
        return Ok(());
    }
    validate_resolved_command_path(root, entry, kind)
        .map_err(|error| PluginError::InvalidManifest(error.to_string()))?;
    Ok(())
}

fn validate_manifest_command_path(
    root: &Path,
    entry: &str,
    kind: &'static str,
) -> Result<PathBuf, PluginManifestValidationError> {
    let entry_path = Path::new(entry);
    if entry_path.is_absolute() {
        return Err(PluginManifestValidationError::AbsolutePathNotAllowed {
            kind,
    ...
    validate_command_path_within_root(root, kind, path)
}
...
fn validate_command_path_within_root(
    root: &Path,
    kind: &'static str,
    path: PathBuf,
) -> Result<PathBuf, PluginManifestValidationError> {
    if !path.exists() {
        return Err(PluginManifestValidationError::MissingPath {
            kind,
            path: path.clone(),
        });
    }
    ...
    let canonical_path =
        path.canonicalize()
            .map_err(|_| PluginManifestValidationError::MissingPath {
                kind,
                path: path.clone(),
            })?;
    if !canonical_path.starts_with(&canonical_root) {
        return Err(PluginManifestValidationError::PathOutsideRoot {
            kind,
            path: canonical_path,
            root: canonical_root,
        });
...
fn parse_install_source(source: &str) -> Result<PluginInstallSource, PluginError> {
    ...
    {
        validate_git_install_source(source)?;
...
fn validate_git_install_source(source: &str) -> Result<(), PluginError> {
    if git_url_contains_embedded_credentials(source) {
        return Err(PluginError::CommandFailed(format!(
            "plugin git source `{source}` must not include embedded credentials"
        )));
    }
    Ok(())
}

fn git_url_contains_embedded_credentials(source: &str) -> bool {
...
fn prepare_plugin_source(
    ...
    let staged_source = materialize_source(source, temp_root)?;
    let cleanup_source = matches!(source, PluginInstallSource::GitUrl { .. });
    let manifest = match load_plugin_from_directory(&staged_source) {
    ...
    let prepared_install_path = temp_root.join(format!("install-{}", unix_time_ms()));
    if let Err(error) = copy_dir_all(&staged_source, &prepared_install_path) {
        if cleanup_source {
            let _ = fs::remove_dir_all(&staged_source);
        }
        let _ = fs::remove_dir_all(&prepared_install_path);
        return Err(error);
    }
    if cleanup_source {
        let _ = fs::remove_dir_all(&staged_source);
...
fn commit_prepared_install(
    ...
    let backup_path = if install_path.exists() {
        let backup_path = temp_root.join(format!("backup-{}", unix_time_ms()));
        fs::rename(install_path, &backup_path).map_err(|error| {
            PluginError::CommandFailed(format!(
                "failed to stage existing install at {} for replacement: {error}",
    ...
    if let Err(error) = fs::rename(prepared_install_path, install_path) {
        let _ = fs::remove_dir_all(prepared_install_path);
        if let Some(backup_path) = backup_path.as_ref() {
            if let Err(restore_error) = fs::rename(backup_path, install_path) {
                return Err(PluginError::CommandFailed(format!(
                    "failed to replace installed plugin at {}: {error}; failed to restore previous install: {restore_error}",
                    install_path.display()
                )));
            }
        }
        return Err(PluginError::CommandFailed(format!(
            "failed to replace installed plugin at {}: {error}",
            install_path.display()
        )));
    }
...
mod tests {
    ...
    fn executable_script_name(stem: &str) -> String {
        if cfg!(windows) {
            format!("{stem}.cmd")
        } else {
            format!("{stem}.sh")
  ...
  "commands": [
    ...
    fn write_lifecycle_plugin(root: &Path, name: &str, version: &str) -> PathBuf {
        ...
        let init_script = executable_script_name("init");
        let shutdown_script = executable_script_name("shutdown");
        write_file(
            root.join("lifecycle").join(&init_script).as_path(),
            lifecycle_script_contents("init").as_str(),
        );
        write_file(
            root.join("lifecycle").join(&shutdown_script).as_path(),
            lifecycle_script_contents("shutdown").as_str(),
        );
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        let script_name = executable_script_name("echo-json");
        let script_path = root.join("tools").join(&script_name);
        write_file(&script_path, tool_script_contents().as_str());
        #[cfg(unix)]
        {
    ...
    #[test]
    fn plugin_tool_manifest_serializes_and_deserializes_canonical_permission_key() {
        let manifest = PluginToolManifest {
            name: "plugin_echo".to_string(),
            description: "Echo JSON".to_string(),
            input_schema: serde_json::json!({ "type": "object" }),
            command: "./tools/echo.sh".to_string(),
            args: Vec::new(),
        ...
        assert_eq!(encoded["requiredPermission"], "workspace-write");
        assert!(encoded.get("required_permission").is_none());

        let decoded: PluginToolManifest = serde_json::from_value(serde_json::json!({
            "name": "plugin_echo",
        ...
        let legacy: PluginToolManifest = serde_json::from_value(serde_json::json!({
            "name": "plugin_echo",
            "description": "Echo JSON",
            "inputSchema": { "type": "object" },
            "command": "./tools/echo.sh",
  ...
  "description": "Registry fallback plugin"
    ...
    #[test]
    fn rejects_duplicate_plugin_install_without_replacing_existing_install() {
        let config_home = temp_dir("duplicate-home");
        let source_root = temp_dir("duplicate-source");
        write_external_plugin(&source_root, "duplicate-demo", "1.0.0");

        let mut manager = PluginManager::new(PluginManagerConfig::new(&config_home));
        let install = manager
            .install(source_root.to_str().expect("utf8 path"))
            .expect("first install should succeed");
        write_external_plugin(&source_root, "duplicate-demo", "2.0.0");
        ...
        assert!(error
            .to_string()
            .contains("already installed; use `/plugins update duplicate-demo@external`"));

        let manifest =
            load_plugin_from_directory(&install.install_path).expect("installed manifest");
        assert_eq!(manifest.version, "1.0.0");
        let registry = manager.load_registry().expect("registry should load");
        assert_eq!(
            registry
                .plugins
                .get("duplicate-demo@external")
    ...
    #[test]
    fn rejects_git_install_sources_with_embedded_credentials() {
        let config_home = temp_dir("credential-home");
        let mut manager = PluginManager::new(PluginManagerConfig::new(&config_home));

        let error = manager
            .install("https://user:secret@example.com/demo.git")
            .expect_err("credential-bearing git url should fail");
        assert!(error
            .to_string()
    ...
    #[test]
    fn accepts_repo_relative_hook_paths_without_dot_prefix() {
        let source_root = temp_dir("relative-hook-source");
        write_file(
            source_root.join("hooks").join("pre.sh").as_path(),
            "#!/bin/sh\nprintf 'pre'\n",
        );
        write_file(
            source_root.join(MANIFEST_RELATIVE_PATH).as_path(),
            "{\n  \"name\": \"relative-hook\",\n  \"version\": \"1.0.0\",\n  \"description\": \"relative hook plugin\",\n  \"hooks\": {\n    \"PreToolUse\": [\"hooks/pre.sh\"]\n  }\n}\n",
        );

        validate_command_path(&source_root, "hooks/pre.sh", "hook")
            .expect("relative path hook should validate");
        assert_eq!(
            resolve_hook_entry(&source_root, "hooks/pre.sh").replace('\\', "/"),
            source_root
                .join("hooks")
                .join("pre.sh")
                .display()
    ...
    #[test]
    fn rejects_hook_paths_that_escape_the_plugin_root() {
        let config_home = temp_dir("escape-home");
        let source_root = temp_dir("escape-source").join("plugin");
        let escaped_hook = source_root
            .parent()
            .expect("plugin root should have a parent")
            .join("shared")
            .join("pre.sh");
        write_file(&escaped_hook, "#!/bin/sh\nprintf 'pre'\n");
        write_file(
            source_root.join(MANIFEST_RELATIVE_PATH).as_path(),
            "{\n  \"name\": \"escape-hook\",\n  \"version\": \"1.0.0\",\n  \"description\": \"escape hook plugin\",\n  \"hooks\": {\n    \"PreToolUse\": [\"../shared/pre.sh\"]\n  }\n}\n",
        );

        let error = load_plugin_from_directory(&source_root).expect_err("path escape should fail");
        match error {
            PluginError::ManifestValidation(errors) => {
                assert!(errors.iter().any(|error| matches!(
                    error,
                    PluginManifestValidationError::PathOutsideRoot { kind, .. } if *kind == "hook"
                )));
            }
        ...
        let install_error = manager
            .install(source_root.to_str().expect("utf8 path"))
            .expect_err("install should reject path escape");
        assert!(install_error
            .to_string()
            .contains("must stay within the plugin root"));
    ...
    #[test]
    fn rejects_absolute_hook_paths_even_within_the_source_tree() {
        let source_root = temp_dir("absolute-hook-source");
        let hook_path = source_root.join("hooks").join("pre.sh");
        write_file(&hook_path, "#!/bin/sh\nprintf 'pre'\n");
        let serialized_hook_path =
            serde_json::to_string(&hook_path.display().to_string()).expect("serialize hook path");
        write_file(
            source_root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
                "{{\n  \"name\": \"absolute-hook\",\n  \"version\": \"1.0.0\",\n  \"description\": \"absolute hook plugin\",\n  \"hooks\": {{\n    \"PreToolUse\": [{serialized_hook_path}]\n  }}\n}}\n"
            )
            .as_str(),
        );

        let error =
            load_plugin_from_directory(&source_root).expect_err("absolute path should fail");
        match error {
            PluginError::ManifestValidation(errors) => {
                assert!(errors.iter().any(|error| matches!(
                    error,
                    PluginManifestValidationError::AbsolutePathNotAllowed { kind, .. }
                    if *kind == "hook"
                )));
            }
            other => panic!("expected manifest validation error, got {other}"),
    ...
    #[test]
    fn preserves_previous_install_when_update_validation_fails() {
        let config_home = temp_dir("update-preserve-home");
        let source_root = temp_dir("update-preserve-source");
        write_external_plugin(&source_root, "preserve-demo", "1.0.0");
        ...
        let install = manager
            ...
            .expect("install should succeed");
        manager
            .disable("preserve-demo@external")
            .expect("disable should succeed");

        write_broken_plugin(&source_root, "preserve-demo");
        let error = manager
            .update("preserve-demo@external")
            .expect_err("broken update should fail");
        assert!(error.to_string().contains("does not exist"));

        let manifest = load_plugin_from_directory(&install.install_path)
            .expect("installed manifest should stay");
        assert_eq!(manifest.version, "1.0.0");
        let registry = manager.load_registry().expect("registry should load");
        let record = registry
            .plugins
            .get("preserve-demo@external")
            .expect("registry entry should survive");
        ...
        assert!(
            !manager
                ...
                .into_iter()
                .find(|plugin| plugin.metadata.id == "preserve-demo@external")
