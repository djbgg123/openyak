...
impl MockAnthropicService {
    ...
    async fn wait_for_shutdown(&mut self) -> io::Result<()> {
        ...
        join_handle.await.map_err(|error| {
            io::Error::other(format!("mock anthropic service task failed: {error}"))
        })?
    }
}
...
fn extract_user_input_content(tool_output: &str) -> String {
    serde_json::from_str::<Value>(tool_output)
        .ok()
        .and_then(|value| {
            value
...
mod tests {
    ...
    async fn shutdown_completes_after_serving_requests() {
        let server = MockAnthropicService::spawn()
            .await
            .expect("mock service should start");

        let request = serde_json::to_string(&MessageRequest {
            model: DEFAULT_MODEL.to_string(),
            max_tokens: 128,
            messages: vec![InputMessage::user_text(format!(
                "{SCENARIO_PREFIX}streaming_text"
            ))],
            system: None,
            tools: None,
            tool_choice: None,
        ...
        let mut socket = TcpStream::connect(server.base_url().trim_start_matches("http://"))
            .await
        ...
        let response_text = String::from_utf8(response).expect("response should be utf-8");
