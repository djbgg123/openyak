...
const STARTER_OPENYAK_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# openyak local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".openyak/settings.local.json", ".openyak/sessions/"];

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RepoInitAssessment {
    pub(crate) missing_artifacts: Vec<&'static str>,
    pub(crate) missing_gitignore_entries: Vec<&'static str>,
}

impl RepoInitAssessment {
    #[must_use]
    pub(crate) fn is_ready(&self) -> bool {
        self.missing_artifacts.is_empty() && self.missing_gitignore_entries.is_empty()
    }

    #[must_use]
    pub(crate) fn missing_items(&self) -> Vec<String> {
        let mut items = self
            .missing_artifacts
            .iter()
        ...
        if !self.missing_gitignore_entries.is_empty() {
            items.push(".gitignore entries".to_string());
        }
        items
    }
}
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    artifacts.push(InitArtifact {
        name: ".openyak/",
        status: ensure_dir(&openyak_dir)?,
    });

    let openyak_json = cwd.join(".openyak.json");
    artifacts.push(InitArtifact {
        name: ".openyak.json",
        status: write_file_if_missing(&openyak_json, STARTER_OPENYAK_JSON)?,
    });
    ...
    let openyak_md = cwd.join("OPENYAK.md");
    let content = render_init_openyak_md(cwd);
    artifacts.push(InitArtifact {
        name: "OPENYAK.md",
        status: write_file_if_missing(&openyak_md, &content)?,
    });
...
#[must_use]
pub(crate) fn assess_repo_init(cwd: &Path) -> RepoInitAssessment {
    let mut missing_artifacts = Vec::new();
    if !cwd.join(".openyak").is_dir() {
        missing_artifacts.push(".openyak/");
    }
    if !cwd.join(".openyak.json").is_file() {
        missing_artifacts.push(".openyak.json");
    }
    if !cwd.join("OPENYAK.md").is_file() {
        missing_artifacts.push("OPENYAK.md");
    }

    let gitignore_path = cwd.join(".gitignore");
    let missing_gitignore_entries = match fs::read_to_string(&gitignore_path) {
        Ok(contents) => GITIGNORE_ENTRIES
            .iter()
            .copied()
            .filter(|entry| !contents.lines().any(|line| line == *entry))
            .collect(),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => GITIGNORE_ENTRIES.to_vec(),
        Err(_) => GITIGNORE_ENTRIES.to_vec(),
    };
...
pub(crate) fn render_init_openyak_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# OPENYAK.md".to_string(),
        String::new(),
    ...
    lines.push("- Keep shared defaults in `.openyak.json`; reserve `.openyak/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `OPENYAK.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{assess_repo_init, initialize_repo, render_init_openyak_md, InitStatus};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("openyak-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        let before = assess_repo_init(&root);
        ...
        assert!(
            report
                .artifacts
                .iter()
                .any(|artifact| artifact.name == ".openyak/"
                    && artifact.status == InitStatus::Created)
        );
        assert!(report.artifacts.iter().any(|artifact| {
            artifact.name == ".openyak.json" && artifact.status == InitStatus::Created
        }));
        assert!(report.artifacts.iter().any(
            |artifact| artifact.name == ".gitignore" && artifact.status == InitStatus::Created
        ));
        assert!(report.artifacts.iter().any(
            |artifact| artifact.name == "OPENYAK.md" && artifact.status == InitStatus::Created
        ));
        assert!(rendered.contains("Init"));
        assert!(root.join(".openyak").is_dir());
        assert!(root.join(".openyak.json").is_file());
        assert!(root.join("OPENYAK.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".openyak.json")).expect("read openyak json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".openyak/settings.local.json"));
        assert!(gitignore.contains(".openyak/sessions/"));
        let openyak_md = fs::read_to_string(root.join("OPENYAK.md")).expect("read openyak md");
        assert!(openyak_md.contains("Languages: Rust."));
        assert!(openyak_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
        assert!(assess_repo_init(&root).is_ready());
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("OPENYAK.md"), "custom guidance\n").expect("write existing openyak md");
        fs::write(root.join(".gitignore"), ".openyak/settings.local.json\n")
            .expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first.artifacts.iter().any(|artifact| {
            artifact.name == "OPENYAK.md" && artifact.status == InitStatus::Skipped
        }));
        let second = initialize_repo(&root).expect("second init should succeed");
        assert!(second.artifacts.iter().any(|artifact| {
            artifact.name == ".openyak/" && artifact.status == InitStatus::Skipped
        }));
        assert!(second.artifacts.iter().any(|artifact| {
            artifact.name == ".openyak.json" && artifact.status == InitStatus::Skipped
        }));
        assert!(second.artifacts.iter().any(|artifact| {
            artifact.name == ".gitignore" && artifact.status == InitStatus::Skipped
        }));
        assert!(second.artifacts.iter().any(|artifact| {
            artifact.name == "OPENYAK.md" && artifact.status == InitStatus::Skipped
        }));
        assert_eq!(
            fs::read_to_string(root.join("OPENYAK.md")).expect("read existing openyak md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".openyak/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".openyak/sessions/").count(), 1);

        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn repo_init_assessment_detects_missing_and_present_artifacts() {
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create root");

        let initial = assess_repo_init(&root);
        assert!(!initial.is_ready());
        assert!(initial.missing_artifacts.contains(&".openyak/"));
        assert!(initial.missing_artifacts.contains(&".openyak.json"));
        assert!(initial.missing_artifacts.contains(&"OPENYAK.md"));
        assert_eq!(
            initial.missing_gitignore_entries,
            vec![".openyak/settings.local.json", ".openyak/sessions/"]
        );

        initialize_repo(&root).expect("init should succeed");

        let after = assess_repo_init(&root);
        assert!(after.is_ready());
        assert!(after.missing_artifacts.is_empty());
        assert!(after.missing_gitignore_entries.is_empty());
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_openyak_md(Path::new(&root));
