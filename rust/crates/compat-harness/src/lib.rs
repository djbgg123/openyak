...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("OPENYAK_UPSTREAM") {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("openyak"));
        candidates.push(ancestor.join("openyak"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("openyak"));
    candidates.push(primary_repo_root.join("reference-source").join("openyak"));
    candidates.push(primary_repo_root.join("vendor").join("openyak"));
    candidates.push(primary_repo_root.join("vendor").join("openyak"));
