...
#[test]
fn provider_client_routes_gpt_models_through_openai_even_with_openyak_auth_present() {
    let _lock = env_lock();
    let _openai_api_key = EnvVarGuard::set("OPENAI_API_KEY", Some("openai-test-key"));
    let _anthropic_api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", Some("openyak-test-key"));

    let client = ProviderClient::from_model("gpt-5.3").expect("gpt model should resolve");
...
fn provider_client_uses_explicit_auth_without_env_lookup() {
    ...
    let client = ProviderClient::from_model_with_default_auth(
        "claude-sonnet-4-6",
        Some(AuthSource::ApiKey("openyak-test-key".to_string())),
    )
    .expect("explicit auth should avoid env lookup");

    assert_eq!(client.provider_kind(), ProviderKind::OpenyakApi);
}
