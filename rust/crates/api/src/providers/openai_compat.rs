...
impl OpenAiSseParser {
    ...
    fn finish(&mut self) -> Result<Vec<ChatCompletionChunk>, ApiError> {
        ...
        parse_sse_frame(&frame).map(|event| event.into_iter().collect())
    }
}
        ...
        if !self.message_started {
            ...
            events.push(StreamEvent::MessageStart(MessageStartEvent {
                message: MessageResponse {
                    ...
                    content: Vec::new(),
                    model: normalize_model_name(chunk.model.as_deref(), &self.model),
                    stop_reason: None,
                    stop_sequence: None,
                    usage: Usage {
                        input_tokens: 0,
...
fn normalize_response(
    ...
    Ok(MessageResponse {
        ...
        content,
        model: normalize_model_name(Some(&response.model), model),
        stop_reason: choice
            .finish_reason
...
fn normalize_model_name(model: Option<&str>, fallback: &str) -> String {
    match model.map(str::trim) {
        Some("" | "model") | None => fallback.to_string(),
...
fn normalize_tool_schema(schema: &Value) -> Value {
    ...
    if normalized.get("type") == Some(&Value::String("object".to_string()))
        && normalized.get("properties").is_none()
    {
        normalized["properties"] = Value::Object(serde_json::Map::new());
    }
    normalized
}
...
fn parse_sse_frame(frame: &str) -> Result<Option<ChatCompletionChunk>, ApiError> {
    ...
    if let Some(error) = parse_sse_error(payload) {
        return Err(error);
    }
...
fn parse_sse_error(payload: &str) -> Option<ApiError> {
    let parsed_error = serde_json::from_str::<ErrorEnvelope>(payload).ok()?;
    if parsed_error.error.is_none()
        && parsed_error.detail.is_none()
        && parsed_error.message.is_none()
    {
        return None;
    }
    ...
    Some(ApiError::Api {
        ...
        message: parsed_error
            ...
            .or(parsed_error.message)
            .or(parsed_error.detail),
...
async fn expect_success(response: reqwest::Response) -> Result<reqwest::Response, ApiError> {
    ...
    Err(ApiError::Api {
        ...
        error_type: parsed_error
            ...
            .and_then(|error| error.error_type.clone()),
        message: parsed_error.as_ref().and_then(|error| {
            error
                .error
                .as_ref()
                .and_then(|inner| inner.message.clone())
...
mod tests {
    use super::{
        ...
        normalize_model_name, normalize_tool_schema, openai_tool_choice, parse_sse_frame,
        parse_tool_arguments, ChatCompletionChunk, ChunkChoice, ChunkDelta, OpenAiCompatClient,
        OpenAiCompatConfig, OpenAiSseParser, StreamState,
    };
    use crate::error::ApiError;
    use crate::types::{
        InputContentBlock, InputMessage, MessageRequest, StreamEvent, ToolChoice, ToolDefinition,
    ...
    #[test]
    fn normalizes_freeform_object_tool_schemas_for_openai_gateways() {
        assert_eq!(
            normalize_tool_schema(&json!({
                "type": "object",
    ...
    #[test]
    fn parse_sse_frame_ignores_empty_data_payloads() {
    ...
    #[test]
    fn parse_sse_frame_surfaces_multiline_gateway_errors() {
        let error = parse_sse_frame(
            "data: {\n  \"error\": {\n    \"message\": \"bad schema\",\n    \"type\": \"invalid_request_error\"\n  }\n}\n\n",
        )
        .expect_err("gateway error frame should surface an api error");
        ...
        assert!(matches!(
            ...
            ApiError::Api {
                ...
                ..
            } if error_type == "invalid_request_error" && message == "bad schema"
        ));
    }

    #[test]
    fn parser_finish_parses_final_frame_without_separator() {
        let mut parser = OpenAiSseParser::new();
        let frame = concat!(
            "data: {",
            "\"id\":\"resp_1\",",
            "\"model\":\"gpt-5.3-codex\",",
            "\"choices\":[{\"delta\":{\"content\":\"OK\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],",
            "\"usage\":{\"prompt_tokens\":1,\"completion_tokens\":1}",
            "}"
        );
        parser
            .push(frame.as_bytes())
            .expect("partial frame should buffer without errors");

        let parsed = parser.finish().expect("finish should parse trailing frame");

        assert_eq!(parsed.len(), 1);
        assert_eq!(parsed[0].id, "resp_1");
    }

    #[test]
    fn parser_accepts_null_tool_calls_from_gateway_chunks() {
        let frame = concat!(
            "data: {",
            "\"id\":\"resp_1\",",
            "\"model\":\"model\",",
            "\"choices\":[{\"delta\":{\"role\":\"assistant\",\"content\":\"OK\",\"tool_calls\":null},\"finish_reason\":null}],",
            "\"usage\":{\"prompt_tokens\":1,\"completion_tokens\":1}",
            "}\n\n"
        );
    ...
    #[test]
    fn stream_state_uses_requested_model_for_placeholder_model_name() {
        let mut state = StreamState::new("gpt-5.3-codex".to_string());
        let events = state.ingest_chunk(ChatCompletionChunk {
            id: "resp_1".to_string(),
        ...
        assert!(matches!(
            &events[0],
            StreamEvent::MessageStart(start) if start.message.model == "gpt-5.3-codex"
        ));
    }
    ...
    fn normalizes_stop_reasons() {
        ...
        assert_eq!(
            normalize_model_name(Some("model"), "gpt-5.3-codex"),
            "gpt-5.3-codex"
        );
    }
}
