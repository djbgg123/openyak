use crate::compact::session_has_compacted_summary;
use crate::session::Session;
use crate::session::SessionAccountingStatus;
...
impl UsageTracker {
    ...
    pub fn from_session(session: &Session) -> Self {
        let mut tracker = Self {
            ...
            cumulative: session
                .telemetry
                .map(|telemetry| telemetry.compacted_usage)
                .unwrap_or_default(),
            turns: session
                .telemetry
                .map_or(0, |telemetry| telemetry.compacted_turns),
            accounting_status: session.telemetry.map_or_else(
                || {
    ...
    #[must_use]
    pub fn accounting_status(&self) -> SessionAccountingStatus {
...
mod tests {
    ...
    #[test]
    fn reconstructs_usage_from_compacted_session_telemetry() {
        let session = Session {
            version: 1,
            messages: vec![ConversationMessage {
                role: MessageRole::Assistant,
        ...
        assert_eq!(tracker.cumulative_usage().input_tokens, 16);
        assert_eq!(tracker.cumulative_usage().output_tokens, 9);
        assert_eq!(tracker.cumulative_usage().cache_read_input_tokens, 2);
        assert_eq!(
            tracker.accounting_status(),
            SessionAccountingStatus::Complete
        );
    ...
    #[test]
    fn marks_legacy_compacted_sessions_as_partial() {
        let session = Session {
            version: 1,
            messages: vec![ConversationMessage {
                role: MessageRole::System,
                blocks: vec![ContentBlock::Text {
                    text: "This session is being continued from a previous conversation that ran out of context. The summary below covers the earlier portion of the conversation.\n\nSummary:\nlegacy\nContinue the conversation from where it left off without asking the user any further questions. Resume directly — do not acknowledge the summary, do not recap what was happening, and do not preface with continuation text.".to_string(),
                }],
                usage: None,
            }],
            telemetry: None,
        ...
        assert_eq!(tracker.cumulative_usage(), TokenUsage::default());
