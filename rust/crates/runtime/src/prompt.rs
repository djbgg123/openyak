...
impl SystemPromptBuilder {
    ...
    fn environment_section(&self) -> String {
        ...
        let model_family = self.model_family.as_deref().unwrap_or(FRONTIER_MODEL_NAME);
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in instruction_directories(cwd) {
        for candidate in [
            dir.join("OPENYAK.md"),
            dir.join("OPENYAK.local.md"),
            dir.join(".openyak").join("OPENYAK.md"),
            dir.join(".openyak").join("instructions.md"),
        ] {
...
fn instruction_directories(cwd: &Path) -> Vec<PathBuf> {
    let temp_root = system_temp_root();
    instruction_directories_with_temp_root(cwd, temp_root.as_deref())
}

fn instruction_directories_with_temp_root(cwd: &Path, temp_root: Option<&Path>) -> Vec<PathBuf> {
    let mut directories = Vec::new();
    let mut cursor = Some(cwd);

    while let Some(dir) = cursor {
        directories.push(dir.to_path_buf());
        if temp_root.is_some_and(|root| paths_match(dir, root)) {
            break;
        }
        ...
        if temp_root.is_some_and(|root| paths_match(parent, root)) {
            break;
        }
        cursor = Some(parent);
    }
...
fn system_temp_root() -> Option<PathBuf> {
    let temp_root = std::env::temp_dir();
    (!temp_root.as_os_str().is_empty()).then_some(canonical_or_original(&temp_root))
}
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "openyak instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# openyak instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No openyak settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    use super::{
        collapse_blank_lines, display_context_path, instruction_directories_with_temp_root,
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".openyak")).expect("nested openyak dir");
        fs::write(root.join("OPENYAK.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("OPENYAK.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".openyak")).expect("apps openyak dir");
        fs::write(root.join("apps").join("OPENYAK.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".openyak").join("instructions.md"),
            "apps dot openyak instructions",
        )
        .expect("write apps dot openyak instructions");
        fs::write(nested.join(".openyak").join("OPENYAK.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".openyak").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("OPENYAK.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("OPENYAK.md"), "same rules\n").expect("write nested");
    ...
    #[test]
    fn instruction_scan_stops_before_system_temp_root_ancestor() {
        let temp_root = std::env::temp_dir();
        let root = temp_root.join("runtime-prompt-scope-root");
        let nested = root.join("apps").join("api");

        let directories = instruction_directories_with_temp_root(&nested, Some(&temp_root));

        assert_eq!(directories, vec![root.clone(), root.join("apps"), nested]);
    }
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.openyak/OPENYAK.md")),
            "OPENYAK.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("OPENYAK.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? OPENYAK.md"));
    ...
    #[test]
    fn load_system_prompt_reads_openyak_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".openyak")).expect("openyak dir");
        fs::write(root.join("OPENYAK.md"), "Project rules").expect("write instructions");
        fs::write(
        ...
        let original_openyak_home = std::env::var("OPENYAK_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("OPENYAK_CONFIG_HOME", root.join("missing-home"));
        std::env::set_current_dir(&root).expect("change cwd");
        let prompt =
            super::load_system_prompt(&root, "2026-03-31", "gpt-5.3-codex", "linux", "6.8")
...
",
        ...
        if let Some(value) = original_openyak_home {
            std::env::set_var("OPENYAK_CONFIG_HOME", value);
        } else {
        ...
        assert!(prompt.contains("Model family: gpt-5.3-codex"));
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }

    #[test]
    fn renders_openyak_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".openyak")).expect("openyak dir");
        fs::write(root.join("OPENYAK.md"), "Project rules").expect("write OPENYAK.md");
        fs::write(
            root.join(".openyak").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# openyak instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_openyak_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".openyak")).expect("nested openyak dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".openyak/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/OPENYAK.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# openyak instructions"));
