...
mod tests {
    ...
    fn write_echo_script() -> PathBuf {
        ...
        fs::write(
            ...
            [
                "#!/usr/bin/env python3",
                "import os, sys",
                "sys.stdout.write(f\"READY:{os.environ.get('MCP_TEST_TOKEN', '')}\\n\")",
                "sys.stdout.flush()",
    ...
    fn sample_bootstrap(script_path: &Path) -> McpClientBootstrap {
        let (command, args) = python_script_invocation(script_path);
    ...
    fn python_launcher() -> PythonLauncher {
        ...
        #[cfg(not(windows))]
        let candidates = [("python3", Vec::new()), ("python", Vec::new())];

        for (command, args) in candidates {
            if probe_python_command(command, &args) {
                return PythonLauncher {
    ...
    fn probe_python_command(command: &str, args: &[&str]) -> bool {
        match Command::new(command).args(args).arg("--version").output() {
            Ok(output) => output.status.success(),
            Err(_) => false,
        }
    }

    fn python_script_invocation(script_path: &Path) -> (String, Vec<String>) {
    ...
    fn direct_spawn_uses_transport_env() {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                String::from_utf8_lossy(&ready).replace("\r\n", "\n"),
                "READY:direct-secret\n"
            );
