...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionSummaryMode {
    Unchanged,
    NewSummary,
    MergedExisting,
}
...
pub struct CompactionResult {
    ...
    pub summary_mode: CompactionSummaryMode,
...
#[must_use]
pub(crate) fn session_has_compacted_summary(session: &Session) -> bool {
    compacted_summary_prefix_len(session) > 0
}
...
pub fn compact_session(session: &Session, config: CompactionConfig) -> CompactionResult {
    ...
    let accounting_status = effective_accounting_status(session);
    ...
    let summary_mode = if existing_summary.is_some() {
        CompactionSummaryMode::MergedExisting
    } else {
        CompactionSummaryMode::NewSummary
    };
    ...
    let removed_usage = usage_for_messages(removed);
    let removed_turns = turns_for_messages(removed);
    let summary =
    ...
    let previous_telemetry = session.telemetry.unwrap_or_default();
    let compacted_session = Session {
        version: session.version,
        messages: compacted_messages,
        telemetry: Some(SessionTelemetry {
            compacted_usage: add_usage(previous_telemetry.compacted_usage, removed_usage),
            compacted_turns: previous_telemetry.compacted_turns + removed_turns,
            accounting_status,
        }),
    };
...
fn effective_accounting_status(session: &Session) -> SessionAccountingStatus {
    session.telemetry.map_or_else(
        || {
            if session_has_compacted_summary(session) {
                SessionAccountingStatus::PartialLegacyCompaction
            } else {
                SessionAccountingStatus::Complete
            }
        },
        |telemetry| telemetry.accounting_status,
    )
}

fn usage_for_messages(messages: &[ConversationMessage]) -> TokenUsage {
    messages
        .iter()
        .filter_map(|message| message.usage)
        .fold(TokenUsage::default(), add_usage)
}

fn turns_for_messages(messages: &[ConversationMessage]) -> u32 {
    u32::try_from(
        messages
            .iter()
...
fn summarize_block(block: &ContentBlock) -> String {
    let raw = match block {
        ...
        ContentBlock::UserInputRequest {
            ...
            format!("user_input_request {prompt}{suffix}")
        }
        ContentBlock::UserInputResponse { content, .. } => {
            format!("user_input_response {content}")
        }
    };
...
fn collect_key_files(messages: &[ConversationMessage]) -> Vec<String> {
    let mut files = messages
        ...
        .map(|block| match block {
            ...
            ContentBlock::UserInputRequest { prompt, .. } => prompt.as_str(),
            ContentBlock::UserInputResponse { content, .. } => content.as_str(),
        })
...
fn first_text_block(message: &ConversationMessage) -> Option<&str> {
    message.blocks.iter().find_map(|block| match block {
        ...
        ContentBlock::UserInputResponse { content, .. } if !content.trim().is_empty() => {
            Some(content.as_str())
        }
...
fn estimate_message_tokens(message: &ConversationMessage) -> usize {
    message
        ...
        .map(|block| match block {
            ...
            ContentBlock::UserInputRequest {
                prompt, options, ..
            } => (prompt.len() + options.join("").len()) / 4 + 1,
            ContentBlock::UserInputResponse { content, .. } => content.len() / 4 + 1,
        })
        .sum()
}
...
mod tests {
    ...
    fn leaves_small_sessions_unchanged() {
        ...
        assert_eq!(result.summary_mode, CompactionSummaryMode::Unchanged);
        assert_eq!(
    ...
    fn compacts_older_messages_into_a_system_summary() {
        ...
        assert_eq!(result.summary_mode, CompactionSummaryMode::NewSummary);
        assert_eq!(
        ...
        assert_eq!(
            result.compacted_session.telemetry,
            Some(SessionTelemetry {
                compacted_usage: TokenUsage::default(),
                compacted_turns: 0,
    ...
    fn keeps_previous_compacted_context_when_compacting_again() {
        ...
        let second = compact_session(
            &Session {
                ...
                telemetry: first.compacted_session.telemetry,
            },
            config,
        );
        ...
        assert_eq!(second.summary_mode, CompactionSummaryMode::MergedExisting);
    }
    ...
    #[test]
    fn preserves_accounting_from_removed_messages() {
        let session = Session {
            version: 1,
            messages: vec![
    ...
    #[test]
    fn retains_partial_accounting_for_legacy_compacted_sessions() {
        let session = Session {
            version: 1,
            messages: vec![
                ConversationMessage {
                ...
                ConversationMessage::user_text("carry this forward"),
            ],
            telemetry: None,
        };
