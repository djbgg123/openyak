...
- 当前 `/v1/threads` 已具备最小持久化与 restart-recovery 原语：如果 server 在 run 中途重启，线程快照会恢复成 `interrupted` 状态并附带 `recovery_note`；这是 daemon/control-plane 演进的基础，但还不是名为 `daemon_local_v1` 之类的独立 daemon truth layer。
...
- 已有：thread 级 durable snapshot、restart 后的 `interrupted` + `recovery_note`、attach-first SDK reconnect / resync 语义。
...
- daemon/control-plane 路线仍未发货独立的 service install/start/stop/status/recover surface；现在的恢复语义只覆盖 thread attach-first compatibility，不等同于完整 daemon lifecycle management。
