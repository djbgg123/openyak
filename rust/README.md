...
最近一次全量文档与命令面对齐完成于 `2026-04-08`。本文内容已对照当前 `openyak` CLI help、release-binary 逐命令巡检，以及工作区和 SDK 本地验证结果更新。
...
- `openyak foundations [family]` 与 `openyak /foundations [family]` 已提供只读的 operator discovery surface，用于解释当前 Task / Team / Cron / LSP / MCP 五族的 tool membership 与边界。
...
- `openyak foundations [family]` / `/foundations [family]` 作为只读的 discovery surface，明确说明当前 Task / Team / Cron / LSP / MCP 的 tool membership 与 `process_local_v1` / registry-backed 边界
