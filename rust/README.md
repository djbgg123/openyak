...
最近一次全量文档与命令面对齐完成于 `2026-04-10`。本文内容已对照当前 `openyak` CLI help、`2026-04-10` 完成的 fresh release-binary 全量直接命令矩阵（本轮包含 47 个 release-binary help/命令/子命令步骤，并补做 REPL、server API 与恢复路径复核）与 CLI smoke/regression rerun 更新；工作区、根目录 Python、Python SDK、TypeScript SDK 的仓库级全量验证基线仍以 `2026-04-09` 的结果为准。
...
- `openyak doctor` 已提供本地只读的 config/auth/runtime 健康检查入口，并开始回显当前 workspace 的 local daemon/thread-server readiness。
