...
- `openyak server status` 现在除了 discovery/liveness 外，也会回显 staged install bundle truth；JSON 输出会显式区分 `missing` / `bundle_staged` / `invalid_manifest`。
- `openyak server status` 现在也会回显 local MCP operator bootstrap truth；`capabilities.mcp` 会显式区分 `ready` / `auth_required` / `degraded` / `disabled`，并列出每个 configured MCP server 的 transport、auth_state 与顶层 recommended_actions；这是一层 local bootstrap/readiness 观察，不是假装已经建立了 live MCP session。
- `openyak server start --detach` / `recover` / `stop` 现在也共享稳定的 text/json operator contract：都会回显 `contract`、`lifecycle` 与 local bootstrap `capabilities.mcp`；其中 `stop_failed` 会落到 `failure_kind = daemon_local_stop_failed` 与 `recovery_kind = inspect_error_and_retry`，只描述当前工作区 local operator 的恢复建议。
...
- `openyak doctor` 已提供本地只读的 config/auth/runtime 健康检查入口，并开始回显当前 workspace 的 local daemon/thread-server readiness 与 staged service-install guidance。
- 当 workspace local daemon 已可达、但 MCP capability 仍处于 `auth_required` 或 `degraded` 时，`openyak doctor` 会把 local daemon 条目降级为 warning，而不是把 daemon reachability 误报为完全健康。
...
`openyak onboard` 只在交互式本地终端中运行；它不会改变 `openyak` 无参数默认进入 REPL 的语义，也不会把 provider secrets 写进配置文件。在非交互终端中调用时，它会直接拒绝执行，并回显当前 workspace local daemon 的 install-aware 下一步建议，包括 `openyak server install` 的 bundle path 和显式 activation/status guidance。
...
- `openyak doctor` 对配置加载、OAuth 配置/凭据、活动模型鉴权预检、当前 workspace local daemon/thread-server readiness，以及 GitHub CLI 可用性 / `gh auth status` readiness 做本地只读检查
- `openyak doctor` 在 local daemon running 但 MCP capability 未 ready 时，会复用 `openyak server status` 同一份 bootstrap capability truth，输出 warning 和对应的 auth/config handoff
...
- `openyak server install` 生成 user-scoped、可逆的本地 service bundle，并回显显式 activation/removal commands；当前仍是 bundle-only，不会自动注册平台 daemon
...
- `openyak server status` 作为首个最小 CLI-first daemon/operator inspection：只读回显当前工作区 thread server 的 discovery、reachability、`daemon_local_v1` contract labels、本地状态库存在性，以及 local MCP operator capability readiness/degraded/auth_required/disabled truth
- `openyak server stop` 作为第一个 local-only operator action：只会在验证 reachable operator identity 与当前工作区 discovery/pid 一致后停止本地 thread server，并在 stale registration 场景下清理旧发现记录；其 text/json 输出现在也与 `start --detach` / `recover` 对齐，稳定回显 `contract` / `lifecycle` / `capabilities.mcp`，其中 `stop_failed` 会使用 `daemon_local_stop_failed` + `inspect_error_and_retry` 的 typed recovery metadata
...
- 已有：`openyak server install` 作为 bundle-only 的最小 CLI-first local service-management install surface；只在 `OPENYAK_CONFIG_HOME` 下 staging 用户级 install bundle 与激活/移除命令。
- 已有：`openyak server status` 作为只读、local-only 的最小 CLI-first daemon operator inspection。
- 已有：`openyak server stop` 作为第一个最小 local-only operator action，但仍然只覆盖当前工作区 thread server；其输出 contract 已与 `start --detach` / `recover` 对齐到同一套 `contract` / `lifecycle` / `capabilities.mcp` schema。
- 已有：`openyak server recover` 作为第一个最小 local-only recovery action，只在当前工作区 `daemon_local_v1` thread truth 可安全恢复时执行。
- 未有：daemon-backed worker/task/team truth layer、跨 family 的 daemon lifecycle store、自动注册型平台 daemon installer，或超出当前 thread server install/start/status/stop/recover 的 CLI-first operator controls。
...
- daemon/control-plane 路线当前已发货 bundle-only 的 `openyak server install`，但它只负责在 `OPENYAK_CONFIG_HOME` 下 staging 用户级 install bundle 与显式 activation/removal commands；当前仍只有 local-only `openyak server install/start --detach/status/stop/recover`，恢复语义也仍只覆盖 thread attach-first compatibility，不等同于完整 daemon lifecycle management。
