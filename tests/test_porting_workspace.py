...
class PortingWorkspaceTests(unittest.TestCase):
    ...
    def test_session_store_defaults_to_temp_directory_not_repo_root(self) -> None:
        from src.session_store import DEFAULT_SESSION_DIR

        self.assertEqual(DEFAULT_SESSION_DIR, Path(tempfile.gettempdir()) / 'openyak-port-sessions')
        self.assertNotEqual(DEFAULT_SESSION_DIR, Path('.port_sessions'))

    def test_session_store_rejects_path_traversal_session_ids(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(ValueError):
                load_session('../escape', directory=directory)
