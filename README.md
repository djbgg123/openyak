...
最近一次全量文档与命令面对齐完成于 `2026-04-08`。本文内容已对照当前 `openyak --help` / `openyak skills help` / `openyak foundations --help` / `openyak server --help`、最新 release-binary 逐命令巡检，以及 Rust、Python、TypeScript 三条本地验证链路收口。
...
- 最近一次 fresh release-binary 命令面巡检完成于 `2026-04-08`，共覆盖 58 个真实 release-binary help/命令/子命令步骤，包含顶层 help、直接命令、skills lifecycle、direct slash CLI、resume-safe slash command 链路，以及环境依赖路径的受控失败。
...
- `openyak foundations [family]` 与 `openyak /foundations [family]` 已提供只读的 operator discovery surface，用于解释当前 Task / Team / Cron / LSP / MCP 五族的 tool membership 与边界，而不把它们伪装成更宽的控制面。
...
- 最近一轮 fresh release-binary CLI command-surface 巡检已在 `2026-04-08` 完成，顶层 help 路由、逐个顶层命令 help、直接命令、skills lifecycle、direct slash CLI、resume-safe slash command 链路，以及 `login` / `onboard` / `prompt` / self-target `package-release` 的受控失败路径现在都有可重复回归保护。
...
- `openyak agents`、`openyak skills`、`openyak skills available/info/install/update/uninstall`、`openyak foundations [family]`、`dump-manifests`、`bootstrap-plan`、`system-prompt`、`login`、`logout`、`init`、`doctor`、`server`、`package-release` 等顶层命令
- `openyak /agents`、`openyak /skills`、`openyak /foundations [family]` 这类 direct slash CLI 入口，以及 `openyak --resume SESSION.json ...` 的 resume-safe slash command 链路
...
另外还做过一轮在 `2026-04-08` 完成的 fresh release binary 级命令面巡检，覆盖：
